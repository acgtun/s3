...
void CKlongestPathDP::insert(AdjLinkDP * adj, int &adjN, int a, int b, int c) {
...
int CKlongestPathDP::Query(int u, int k) {
	if (dp[u][0] >= k)
		return dp[u][k];
	//printf("%d %d\n", u, k);
	if (dp[u][0] == 0) {
...
void CKlongestPathDP::ShowPath(int n, int k, vector<size_t> & pathTmp) {
	if (pre[n][k] == -1) {
		pathTmp.push_back(n);
		return;
	}
...
bool CKlongestPathDP::FindCandidiate(char * strCandPep, const int & i, int & rawScore) {
	rawScore = Query(nNumNode - 1, i + 1);
	if (rawScore != INT_MIN) {
		vector<size_t> pathTmp;
		ShowPath(nNumNode - 1, i + 1, pathTmp);
		for (size_t i = 1; i < pathTmp.size() - 1; i++) {
...
int CKlongestPathDP::GetWeight(const char & queryAA, const char & AA) {
	if (queryAA == ' ')
		return 0;
	return BLOSUM62[base[AA - 'A']][base[queryAA - 'A']];
}

void CKlongestPathDP::UpdateWeight(const char * querySeq) {
	/* set edges' weight */
	for (int i = 0; i < nNumNode; i++) {
		for (size_t j = 0; j < vLinkNodeSize[i]; j++) {
			vEdgeWeight[i][j] = GetWeight(NODELABEL[vNodeLabel[vLinkList[i][j]]], querySeq[(i + 19) / 20]);
		}
	}
