...
void Evalue::UpdateValues(const usint32_t& query_len) {
  expected_HSP_length_ungapped = (logK_ungapped + log_database_size_
      + log(query_len)) / H_ungapped;
  expected_HSP_length_gapped = (logK_gapped + log_database_size_
      + log(query_len)) / H_gapped;
  ...
  effective_length_database_ungapped = database_size_
      - num_of_sequences_in_database_ * expected_HSP_length_ungapped;
  if (effective_length_database_ungapped < 1 / K_ungapped) {
    effective_length_database_ungapped = 1 / K_ungapped;
  }

  effective_length_database_gapped = database_size_
      - num_of_sequences_in_database_ * expected_HSP_length_gapped;
  if (effective_length_database_gapped < 1 / K_gapped) {
    effective_length_database_gapped = 1 / K_gapped;
  }

  score_by_evalue_[0] = (log(effective_length_database_ungapped)
      + log(effective_length_query_ungapped) + logK_ungapped - logE)
      / lambda_ungapped;
  score_by_evalue_[1] = (log(effective_length_database_gapped)
...
//  cout << "K_gapped = " << K_gapped << " "
//      << effective_length_database_gapped << " " << effective_length_query_gapped
//          << endl;
